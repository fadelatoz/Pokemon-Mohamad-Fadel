...
const Pokemon: React.FC<PokemonProps> = ({ name, image,id, innerRef }) => {
  ...
  return (
    <React.Fragment>
      ...
      >
        <div className='flex flex-row cursor-pointer shadow-lg bg-slate-900 dark:bg-white border-r-4  gap-4 items-center p-2 rounded'>
          ...
          <div><span data-testid = {'pokemon-name'} className='text-white dark:text-black'>{name}</span></div>

          <div><span className='text-white dark:text-black'>#0{id + 1}</span></div>
        </div>
      </div>
    </React.Fragment>
  )
